...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Carrega produtos dinamicamente dos arquivos
  if (countBadge) {
    countBadge.textContent = 'Carregando catálogo de produtos...';
  }

  await window.ProductManager.loadProducts();

  function renderProducts() {
    const products = window.ProductManager.getFilteredProducts(currentCategory, currentSearch);
    ...
    if (products.length === 0) {
      container.innerHTML = `
        <div class="catalog-empty">
          ...
          <button type="button" class="btn btn-whatsapp sm" onclick="document.getElementById('product-search').value=''; window.resetFilter();">
    ...
    container.innerHTML = products.map(prod => {
      ...
      const catObj = window.PRODUCT_CATEGORIES.find(c => c.id === prod.categoria) || { nome: 'Geral' };
      const isCardOnly = prod.apenasCard === true || prod.semBotao === true || prod.mostrarBotao === false;
      ...
      return `
        <article class="product-card fade-in" id="${escapeHtml(prod.id || '')}">
          <div class="product-card-img-wrap">
            ...
            <img src="${escapeHtml(prod.imagem || 'assets/images/produtos/golden.webp')}" 
                 alt="${escapeHtml(prod.nome)}" 
                 class="product-card-img"
                 loading="lazy"
                 onerror="this.src='assets/images/produtos/golden.webp'">
          </div>
          <div class="product-card-body">
            <span class="product-category-chip">${escapeHtml(prod.subcategoria || catObj.nome)}</span>
